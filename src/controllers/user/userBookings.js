...
// controller (avoids circular requires).
async function loadActiveHiringsForVendor(vendorId) {
  try {
    if (!vendorId) return [];
    return await UserBooking.find({
      "assignedProfessional.professionalId": String(vendorId),
      "bookingDetails.status": {
        $in: Array.from(VENDOR_ACTIVE_PROJECT_STATUSES),
      },
      $or: [
    ...
    console.error("loadActiveHiringsForVendor error:", e);
    return [];
  }
}
      ...
      for (const lead of arr) {
        ...
        if (type === "hp") {
          ...
          if (
            ...
            prof = {
              ...invited,
              name: lead.assignedProfessional.name,
              acceptedDate: lead.assignedProfessional.acceptedDate,
              startedDate: lead.assignedProfessional.startedDate,
            };
          } else {
            prof = invited;
          }
...
exports.getCustomerPaymentsForMD = async (req, res) => {
  try {
    ...
    const paymentDetails = bookings.flatMap((booking) => {
      return (booking.payments || [])
        .filter((p) => {
          // ignore site visit
          if (p?.purpose === "site_visit") return false;

          // must have method + amount
          if (!p?.method || !Number(p?.amount)) return false;
        ...
        .map((p) => {
          const paidAt = p.at ? new Date(p.at) : null;
          const paidDate = paidAt ? moment(paidAt).format("DD/MM/YYYY") : "";
          const paidTime = paidAt ? moment(paidAt).format("hh:mm:ss A") : "";
