...
//     // search can match serviceArea OR address.location OR vendor name (optional)
...
//       filter.$or = [
//         { "vendor.vendorName": rx },
//         // { "vendor.serviceArea": rx },
//         // { "address.location": rx },
//       ];
//     }
...
//         .skip(skip)
//         .limit(limit),
//       vendorAuthSchema.countDocuments(filter),
//     ]);
...
const normalizeMobile = (v) => String(v ?? "").replace(/\D/g, "").trim();
const isValidMobile = (m) => m.length === 10; // adjust if needed
...
// ✅ run with/without transaction (optimized for large DB)
const withTxn = async (fn) => {
  ...
  try {
    ...
    // If transactions not supported (standalone mongo), fallback
    ...
    if (
      ...
      session.endSession();
      return fn(null); // fallback without session
    }
    throw err;
  } finally {
    try {
...
// ✅ reserve a mobile in registry
const reserveMobile = async ({ mobileNumber, ownerType, vendorId, memberId, session }) => {
  const doc = { mobileNumber, ownerType, vendorId, memberId: memberId || null };
  return MobileRegistry.create([doc], session ? { session } : undefined);
};

// ✅ change registry mobile (atomic style: delete old then insert new)
const changeMobile = async ({ oldMobile, newMobile, ownerType, vendorId, memberId, session }) => {
  const delQuery = { mobileNumber: oldMobile, ownerType, vendorId };
  if (ownerType === "team") delQuery.memberId = memberId;

  if (session) {
    await MobileRegistry.deleteOne(delQuery).session(session);
    await reserveMobile({ mobileNumber: newMobile, ownerType, vendorId, memberId, session });
  } else {
    await MobileRegistry.deleteOne(delQuery);
    await reserveMobile({ mobileNumber: newMobile, ownerType, vendorId, memberId, session: null });
  }
};

// ✅ CREATE VENDOR (global unique mobile)
exports.createVendor = async (req, res) => {
  try {
    const vendor = safeJson(req.body.vendor);
    const documents = safeJson(req.body.documents);
    ...
    const mobile = normalizeMobile(vendor.mobileNumber);
    if (!mobile || !isValidMobile(mobile)) {
      return res.status(400).json({ message: "Valid 10-digit mobileNumber is required" });
    }
    ...
    const result = await withTxn(async (session) => {
      ...
      if (session) await newVendor.save({ session });
      else await newVendor.save();

      // 2) reserve mobile in registry (unique index)
      await reserveMobile({
        mobileNumber: mobile,
        ownerType: "vendor",
    ...
    return res.status(201).json({ message: "Vendor account created!", newVendor: result });
  } catch (error) {
    // ✅ duplicate mobile
    if (error?.code === 11000) {
...
// ✅ UPDATE VENDOR (if mobile change => update registry)
exports.updateVendor = async (req, res) => {
  try {
    const { vendorId } = req.params;

    if (!vendorId || !mongoose.Types.ObjectId.isValid(vendorId)) {
      return res.status(400).json({ status: "fail", message: "Valid vendorId is required" });
    }
    ...
    // vendor fields (except mobile handled separately)
    setIfPresent(update, vendor, "vendorName", "vendor.vendorName");
    setIfPresent(update, vendor, "dateOfBirth", "vendor.dateOfBirth");
    setIfPresent(update, vendor, "yearOfWorking", "vendor.yearOfWorking");
    setIfPresent(update, vendor, "city", "vendor.city");
    setIfPresent(update, vendor, "serviceType", "vendor.serviceType");
    setIfPresent(update, vendor, "capacity", "vendor.capacity", Number);
    setIfPresent(update, vendor, "serviceArea", "vendor.serviceArea");
    ...
    setIfPresent(update, bankDetails, "ifscCode", "bankDetails.ifscCode");
    setIfPresent(update, bankDetails, "bankName", "bankDetails.bankName");
    setIfPresent(update, bankDetails, "branchName", "bankDetails.branchName");
    setIfPresent(update, bankDetails, "holderName", "bankDetails.holderName");
    setIfPresent(update, bankDetails, "accountType", "bankDetails.accountType");
    setIfPresent(update, bankDetails, "gstNumber", "bankDetails.gstNumber");
    ...
    // files
    const files = req.files || {};
    if (files.profileImage?.[0]) update["vendor.profileImage"] = files.profileImage[0].path;
    if (files.aadhaarfrontImage?.[0]) update["documents.aadhaarfrontImage"] = files.aadhaarfrontImage[0].path;
    if (files.aadhaarbackImage?.[0]) update["documents.aadhaarbackImage"] = files.aadhaarbackImage[0].path;
    if (files.panImage?.[0]) update["documents.panImage"] = files.panImage[0].path;
    if (files.otherPolicy?.[0]) update["documents.otherPolicy"] = files.otherPolicy[0].path;

    // ✅ mobile change
    const oldMobile = normalizeMobile(existing?.vendor?.mobileNumber);
    let newMobile = null;

    if (hasOwn(vendor, "mobileNumber") && !isIgnorable(vendor.mobileNumber)) {
      newMobile = normalizeMobile(vendor.mobileNumber);
      if (!isValidMobile(newMobile)) {
        return res.status(400).json({ status: "fail", message: "Valid 10-digit mobileNumber required" });
      }
      if (newMobile !== oldMobile) {
        update["vendor.mobileNumber"] = newMobile;
      }
    }

    if (Object.keys(update).length === 0 && !newMobile) {
      return res.status(200).json({ status: "success", message: "No changes detected", vendor: existing });
    }

    const updated = await withTxn(async (session) => {
      // If mobile changed, update registry first (so unique constraint blocks early)
      ...
      const doc = session
        ? await Vendor.findByIdAndUpdate(vendorId, { $set: update }, { new: true, runValidators: true, session })
        : await Vendor.findByIdAndUpdate(vendorId, { $set: update }, { new: true, runValidators: true });

      return doc;
    });

    return res.status(200).json({ status: "success", message: "Vendor updated successfully", vendor: updated });
  } catch (err) {
    if (err?.code === 11000) {
...
// ✅ ADD TEAM MEMBER (global unique mobile)
exports.addTeamMember = async (req, res) => {
  try {
    const vendorId = req.body.vendorId;
    if (!vendorId || !mongoose.Types.ObjectId.isValid(vendorId)) {
    ...
    if (!mobile || !isValidMobile(mobile)) {
      return res
        .status(400)
        .json({ message: "Valid 10-digit mobileNumber is required" });
    }
    ...
    const memberId = new mongoose.Types.ObjectId(); // ✅ fixed id upfront
    ...
    await withTxn(async (session) => {
      // 1) reserve mobile (unique, indexed)
      if (session) {
      ...
      // 2) push team member (do NOT return full vendor)
      const upd = session
      ...
      if (!upd?.matchedCount) {
        // rollback registry if vendor missing (rare because we checked exists)
        if (session) await MobileRegistry.deleteOne({ mobileNumber: mobile }).session(session);
        else await MobileRegistry.deleteOne({ mobileNumber: mobile });
        throw new Error("Vendor not found");
      }
    });

    // ✅ Fetch ONLY the new member (small response, fast)
    const onlyNew = await Vendor.findOne(
      { _id: vendorId, "team._id": memberId },
      { "team.$": 1 }
    ).lean();

    return res.status(200).json({
      message: "Team member added",
      member: onlyNew?.team?.[0] || null,
    });
  } catch (err) {
    if (err?.code === 11000) {
      return res.status(409).json({ message: "Mobile number already exists" });
...
// ✅ UPDATE TEAM MEMBER (if mobile change => update registry)
exports.updateTeamMember = async (req, res) => {
  try {
    const { vendorId, memberId } = req.body;
    ...
    const oldMobile = normalizeMobile(teamMember.mobileNumber);
    let newMobile = null;
    ...
    // ✅ mobile change (registry update)
    if (hasOwn(member, "mobileNumber") && !isIgnorable(member.mobileNumber)) {
      newMobile = normalizeMobile(member.mobileNumber);
      if (!isValidMobile(newMobile)) {
        return res.status(400).json({ message: "Valid 10-digit mobileNumber required" });
      }
      // update in memory; final save after registry success
      if (newMobile !== oldMobile) {
        teamMember.mobileNumber = newMobile;
      }
    }
    ...
    // bank
    if (hasOwn(bankDetails, "accountNumber") && !isIgnorable(bankDetails.accountNumber))
      teamMember.bankDetails.accountNumber = bankDetails.accountNumber;
    if (hasOwn(bankDetails, "ifscCode") && !isIgnorable(bankDetails.ifscCode))
      teamMember.bankDetails.ifscCode = bankDetails.ifscCode;
    if (hasOwn(bankDetails, "bankName") && !isIgnorable(bankDetails.bankName))
      teamMember.bankDetails.bankName = bankDetails.bankName;
    if (hasOwn(bankDetails, "branchName") && !isIgnorable(bankDetails.branchName))
      teamMember.bankDetails.branchName = bankDetails.branchName;
    if (hasOwn(bankDetails, "holderName") && !isIgnorable(bankDetails.holderName))
      teamMember.bankDetails.holderName = bankDetails.holderName;
    if (hasOwn(bankDetails, "accountType") && !isIgnorable(bankDetails.accountType))
      teamMember.bankDetails.accountType = bankDetails.accountType;
    if (hasOwn(bankDetails, "gstNumber") && !isIgnorable(bankDetails.gstNumber))
      teamMember.bankDetails.gstNumber = bankDetails.gstNumber;
    ...
    // ✅ do NOT touch markedLeaves

    const saved = await withTxn(async (session) => {
      // Update registry if mobile changed
      if (newMobile && newMobile !== oldMobile) {
        await changeMobile({
          oldMobile,
          newMobile,
      ...
      vendor.markModified("team");
      if (session) await vendor.save({ session });
      else await vendor.save();

      return vendor;
    });

    return res.status(200).json({ message: "Team member updated", team: saved.team });
  } catch (err) {
    if (err?.code === 11000) {
      return res.status(409).json({ message: "Mobile number already exists" });
    }
...
exports.removeTeamMember = async (req, res) => {
  try {
    ...
    // 1) Remove team member from vendor
    const upd = await Vendor.updateOne(
      { _id: vendorId },
      { $pull: { team: { _id: memberId } } }
    );
    ...
    // 2) ✅ Delete ONLY this team member entry from mobile registry
    await MobileRegistry.deleteOne({
      ownerType: "team",
      vendorId,
      memberId,
    });
