...
const { CATALOG, getSubcategories, getServices, isValidCombo } = require("../../data/deepCleaningCatalog");
...
exports.createPackage = async (req, res) => {
  try {
    ...
    return res.status(400).json({ success: false, message: err.message || "Failed to create package." });
  }
};
...
exports.listPackages = async (req, res) => {
  try {
    ...
    const [items, total] = await Promise.all([
      DeepCleaningPackage.find(filter).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)),
    ...
    res.status(400).json({ success: false, message: err.message || "Failed to list packages." });
  }
};
...
exports.getPackageById = async (req, res) => {
  try {
    ...
    res.status(400).json({ success: false, message: err.message || "Failed to fetch package." });
  }
};
  ...
  try {
    ...
    if (category || subcategory || service !== undefined) {
      ...
      if (!isValidCombo(newCategory, newSubcategory, newService)) {
        return res.status(400).json({ success: false, message: "Invalid category/subcategory/service combination." });
      }
    }

    const updated = await DeepCleaningPackage.findByIdAndUpdate(
      req.params.id,
      { category, subcategory, service, totalAmount, bookingAmount, coinsForVendor, teamMembers },
    ...
    res.status(400).json({ success: false, message: err.message || "Failed to update package." });
  }
};
...
exports.deletePackage = async (req, res) => {
  try {
    ...
    res.status(400).json({ success: false, message: err.message || "Failed to delete package." });
  }
};
