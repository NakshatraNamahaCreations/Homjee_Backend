...
function calculateAvailableSlots({
  ...
  vendors.forEach(v => {
    ...
    console.log(
      `📍 Vendor ${v.vendor?.vendorName || v._id} distance: ${dist.toFixed(2)} km`
    );
  ...
  bookings.forEach(b => {
    ...
    console.log(
      `🔒 Vendor ${vendorId} blocked ${toTime(blockStart)} → ${toTime(blockEnd)}`
    );
  });
  ...
  for (let slotStart = startMin; slotStart <= maxStart; slotStart += 30) {
    ...
    const slotEnd = slotStart + serviceDuration + TRAVEL_MIN;
    if (slotEnd > DAY_TRAVEL_END) continue;
...
//     if (dist > RADIUS_KM) {
//       console.log("   ❌ Outside service radius");
//       reasons.outsideRadius = true;
//       return;
//     }
...
//     (v.team || []).forEach(m => {
//       if (m.markedLeaves?.includes(date)) return;
//       freeMembers.push(m._id.toString());
//     });

//     console.log(`   👥 Available team members: ${freeMembers.length}`);

//     if (freeMembers.length >= minTeamMembers) {
//       vendorResources[v._id.toString()] = freeMembers;
//       console.log("   ✅ VENDOR ELIGIBLE");
//     } else {
//       console.log("   ❌ NOT ENOUGH TEAM");
//     }
//   });
...
//     console.log(`🟢 Vendor ${v} → Team: ${vendorResources[v].length}`)
//   );
...
//     const id = b.assignedProfessional?.teamMemberId;
//     if (!id) return;

//     blocked[id] = blocked[id] || [];
//     blocked[id].push({ start: blockStart, end: blockEnd });

//     console.log(
//       `🔒 BLOCKED MEMBER ${id} → ${toTime(blockStart)} to ${toTime(blockEnd)}`
//     );
//   });
...
//       for (const memberId of vendorResources[vId]) {
//         const blocks = blocked[memberId] || [];
//         const clash = blocks.some(
//           b => slotStart < b.end && slotEnd > b.start
//         );

//         if (!clash) free++;
//         if (free >= minTeamMembers) {
//           console.log(`   🟢 SLOT CAN BE HANDLED BY VENDOR ${vId}`);
//           canServe = true;
//           break;
//         }
//       }
...
//     if (canServe) slots.push(slotLabel);
//     else reasons.allBooked = true;
//   }
